...
### IoT 모트용 확장 모듈
XNode Home에는 IoT 모트의 전용 26핀 확장 커넥터에 연결해 사용하는 5개의 확장 모듈이 포함되어 있습니다. 이 모듈들은 LED, 버튼, 부저를 포함하는 기본 IO 모듈을 비롯하여, 물체의 운동 상태를 파악하는 관성 센서 (IMU: Inertial Measurement Unit), 비접촉 적외선 온도 센서 (IR: Infrared), 움직임 감지 센서, 그리고 범지구 측위 센서로 구성됩니다
...
- IoT 모트의 IO 라인과 전원으로 구성된 터미널 IO 블록 제공
  - IO 단자는 5V 또는 3V3으로 운영되는 입출력 장치 연결
...
- 전원을 사용하는 ON-OFF 형태의 외부 장치(조명, 환기팬 등)를 연결할 수 있도록 공용 접점(Common)과 A접점(normmal open)으로 구성된 3개 채널 터미널 릴레이 블록 제공
  - 2.54mm 2핀 점퍼 쇼트 커넥터를 통해 해당 채널의 공용 접점 단자(이하 C 단자)에 내부 12V 또는 5V 전원을 공급하거나 공급하지 않음 선택
    - 제품 출고 시 1번 채널의 C 단자는 전원에 연결되어 있지 않지만, **2번과 3번 채널의 C 단자는 내부적으로 12V 전원이 연결되어 있음**
- PWM 컨트롤러에 연결하여 최대 2kHz 주파수의 12V PWM 신호를 출력할 수 있는 4개 채널 터미널 PWM 블록 제공
  - 4개의 PWM 채널 단자와 함께 12V 전원 및 GND 단자 포함
- 12V DC 전원으로 운영

**터미널 블록**  
단자대 또는 단자판으로도 불리는 터미널 블록은 전기 회로에서 전선들을 연결하기 위한 모듈식 연결 장치입니다. 간단히 말해, 여러 전선을 안전하고 깔끔하게 연결할 수 있도록 해주는 부품입니다. 특히 산업 현장이나 제어 패널 등에서 배선을 정리하고 유지 보수를 용이하게 하기 위해 널리 사용됩니다. Auto 제어기의 터미널 블록은 기판에 고정하는 고정식 터미널 블록으로 각 단자에 선을 끼운 후 고정할 때는 일자(-) 형 나사를 이용합니다.
