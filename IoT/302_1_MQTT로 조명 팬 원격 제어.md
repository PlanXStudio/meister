...
QObject을 상속한 새 클래스에 paho.mqtt.client.Client의 콜백를 방출하는 시그널을 추가한 후 paho.mqtt.client.Client를 포함하면, QObject의 시그널 처리를 위한 connect()/disconnect()와 paho.mqtt.client.Client의 connect()/disconnect()가 같은 이름을 사용하므로 새 클래스에 브로커 연결을 위한 메소드 이름으로 connect()/disconnect()를 사용할 수 없습니다. 따라서 connection()과 disconnection()을 사용합니다.

다음은 paho-mqtt(V2.0 이상)의 비동기 함수 호출을 PySide6의 신호-슬롯 메커니즘에 통합한 코드입니다.
...
앞서 소개한 PahoMqtt.py를 저장한 폴더에서 다음 명령을 실행해 PahoMqtt.py를 PySide6 라이브러리 경로에 복사합니다. 이렇게하면 여러 프로젝트에서 쉽게 PahoMqtt 모듈을 사용할 수 있습니다.
