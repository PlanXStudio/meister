...
paho-mqtt는 자체적인 이벤트 루프를 사용하여 비동기 함수 호출을 처리하고, PySide6 또한 신호-슬롯 메커니즘을 위한 내부 이벤트 루프를 가지고 있습니다. 이로 인해 두 이벤트 루프가 충돌합니다. 또한 Qt에서 제공하는 QMqtt는 상용 라이선스에서만 사용할 수 있으므로 paho-mqtt의 콜백 처리 방식을 PySide6의 시그널-슬롯 메커니즘으로 변환할 필요가 있습니다.  

paho.mqtt.client.Client의 콜백을 시그널로 방출을 위해 QObject을 상속한 새 클래스 Client에 해당 콜백을 방출하는 Signal()을 추가합니다.
```python
...
새 클래스 생성자에서 paho.mqtt.client.Client에 전달할 매개변수를 정의한 후 paho.mqtt.client.Client 객체를 만든 후 콜백을 등록합니다.
```python
...
해당 콜백이 호출되면 미리 정의한 Signal을 방출합니다.
```python
def __on_connect(self, client, userdata, flags, reason_code, properties):
    self.on_connect.emit(flags, reason_code, properties)
```

이후 브로커 연결 또는 해제를 위한 connect()/disconnect() 메소드를 추가하면, QObject의 시그널 연결/해제릉 위한 connect()/disconnect()와 이름 충돌이 발생하므로 connection()과 disconnection()을 사용합니다.
...
다음은 paho-mqtt(V2.0 이상)의 비동기 함수 호출을 PySide6의 신호-슬롯 메커니즘에 통합한 전체 코드입니다.
...
**PySide6 라이브러리 경로에 PahoMqtt.py 복사**  
앞서 소개한 PahoMqtt.py가 위치한 폴더에서 다음 명령을 실행해 PahoMqtt.py를 PySide6 라이브러리 경로에 복사합니다. 이렇게하면 여러 프로젝트에서 쉽게 PahoMqtt 모듈을 사용할 수 있습니다.
...
### 코드 구현
PySide6 경로에 복사한 PahoMqtt.py와 CondCtrlUi.py를 활용하여 사용자 인터페이스에서 QDial 값이 변경될 때마다 해당 값을 MQTT 토픽 메시지로 발행하는 파이썬 코드를 작성합니다.
