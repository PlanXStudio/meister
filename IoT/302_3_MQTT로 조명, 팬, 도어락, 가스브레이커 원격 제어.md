...
- btGBClose.clicked -> onGBCloseClicked
  - 가스브레이터 닫힘 버튼 선택 
- btLight1.clicked -> onLight1Clicked
  - 조명1 켜짐/꺼짐 버튼 선택 
- btLight2.clicked -> onLight2Clicked
  - 조명2 켜짐/꺼짐 버튼 선택 
...
Fan 속도를 제어하는 dialFan의 sliderReleased와  도어락 상태를 변경하는 btDLChange의 clicked는 토픽 메시지만 발행하므로 람다를 슬롯으로 사용합니다.
...
```python
    def __init__(self):
        ...
        self.btLight1.clicked.connect(lambda clicked: self.onLightClicked(clicked, 1)) 
        self.btLight2.clicked.connect(lambda clicked: self.onLightClicked(clicked, 2)) 
...
**5단계: 가스브레이커 동작 구현** 
가스 브레이커는 Open에서 Close 또는 Close에서 Open까지 약 7초의 동작 시간을 갖습니다. 동작 완료 시 PWM 신호 출력을 중단해야 하며, 사용자가 동작 중에도 언제든 Stop을 통해 동작을 중지할 수 있어야 합니다. 이러한 복잡한 동작 흐름을 효과적으로 관리하기 위해 프로그램 차원의 제어 방안이 필요합니다.  
...
가스브레이커의 최소 상태를 OPEN이며, 이 상태에서 Close 버튼을 선택(CLOSING)하면 dialGB의 value는 증가하고, Open 버튼을 선택()하면 감소해야 합니다.   
따라서 onTimer에서는 isChecked()로 눌러진 버튼을 확인한 후 해당 동작을 수행하는데, 최소값(0: OPEN) 또는 최대값(70: CLOSED)에 도달하면, 가스브레이커의 조작은 중단(CLOSING)되어야 합니다.   

```python
    def onTimer(self):
        n = self.dialGB.value()
        if self.btGBClose.isChecked():
...
```python
    def onLightClicked(self, checked, index):
        self.mqtt.publish("asm/iot/total/light", {"group":index, "action":"on" if checked else "off"})
```

**7단계: 토픽 발행에 따른 결과 구현**  
publish()로 발행한 토픽을 브로커가 수신하면 응답을 MQTT 클라이언트에 전달하는데, 이때마다 클라이언트는 onPublish()를 호출합니다.  
