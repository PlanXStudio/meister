...
"")
    ...
    def retranslateUi(self, MainWindow):
        ...
        self.label.setText(QCoreApplication.translate("MainWindow", u"Open", None))
        self.label_2.setText(QCoreApplication.translate("MainWindow", u"Close", None))
...
**1단계: 클래스 정의**  
일정 시간마다 timeout 시그널을 방출하는 QTimer 모듈을 추가한 후 QMainWindow와 Ui_MainWindow를 상속한 TotalCtrlUi 클래스를 정의합니다.  

생성자에서 edtBroker의 text에 디폴트 브로커 주소를 설정한 후 다음 객체의 시그널에 연결할 슬롯들을 추가합니다. 

- btConDiscon.clicked -> onConDiscon
  - 브로커 연결/해제 요청 버튼 선택 
- tabTotalCtrl.currentChanged -> onTabCurrnetChanged
  - 탭 변경 
- timer.timeout -> onTimer
  - 시간 만료 
- btGBOpen.clicked -> onGBOpenClicked
  - 가스브레이커 열림 버튼 선택 
- btGBStop.clicked -> onGBStopClicked
  - 가스브레이커 제어 중단 버튼 선택 
- btGBClose.clicked -> onGBCloseClicked
  - 가스브레이터 닫힘 버튼 선택 
...
**2단계: 초기화**  
가스브레이커의 상태 전이를 예측하는 타이머 객체를 만든 후 dialGB의 최대값이 7초 * 10 = 70이므로 타이머의 interval은 100ms초 설정했습니다. timer의 timeout 시그널과 가스브레이커의 상태를 바꾸는 btGBOpen, btGBStop, btGBClose의 clicked 시그널에 대한 슬롯을 연결한 후 MQTT 클라이언트 객체를 만들고 on_connect와 on_publish 시그널에 대한 슬롯도 연결합니다.
...
```python
    def __init__(self):
        ...
        self.tabTotalCtrl.currentChanged.connect(self.onTabCurrentChanged)
        ...
        self.btLight1.clicked.connect(lambda checked: self.mqtt.publish("asm/iot/total/light", {"group":1, "action":"on" if checked else "off"})) 
        self.btLight2.clicked.connect(lambda checked: self.mqtt.publish("asm/iot/total/light", {"group":2, "action":"on" if checked else "off"})) 
...
Fan 속도를 제어하는 dialFan의 sliderReleased와 도어락 상태를 변경하는 btDLChange 및 조명 상태를 변경하는 btLight1, btLight2의 clicked는 토픽 메시지만 발행하므로 람다를 사용합니다.
QDial의 valueChanged는 값이 바뀔 때마다 시그널이 발생하지만 sliderReleased는 위젯이 포커스를 잃을 때 한 번 발생합니다.
...
**3단계: 브로커 연결/해제 요청 구현**   
btConDiscon 버튼의 text는 초기에는 "Connection"으로 설정되어 브로커 연결을 유도합니다. 브로커에 연결되면 text는 "Disconnection"으로 변경되어 연결 해제 기능을 제공합니다.  
...
edtBroker에서 읽은 text로 mqtt.connection()을 호출하면, MQTT 클라이언트는 브로커에 연결을 요청한 후 결과를 onConnect()에 전달합니다.  
onConnect()가 호출된 후 reason_code가 0이면 브로커에 연결된 것이므로 btConDiscon의 text를 "Disconnection"으로 바꾸고, edtBroker를 비활성화, tabTotalCtrl은 활성화합니다.
...
**4단계: Tab 변경 동작 구현**   
4개의 탭 중 하나를 선택할 때마다 전달되는 index를 이용해 상태 표시줄에 메시지를 출력합니다.
...
**5단계: 가스브레이커 동작 구현**   
가스브레이커는 Open에서 Close 또는 Close에서 Open까지 약 7초의 동작 시간을 갖습니다. 동작 완료 시 PWM 신호 출력을 중단해야 하며, 사용자가 동작 중에도 언제든 Stop을 통해 동작을 중지할 수 있어야 합니다. 이러한 복잡한 동작 흐름을 효과적으로 관리하기 위해 프로그램 차원의 제어 방안이 필요합니다.  
가스브레이커 제어를 효과적으로 관리하기 위해 유한 상태 머신(a finite state machine: FSM) 기반의 프로그램을 구현하는 것이 좋습니다. 상태 머신은 시스템의 상태 변화를 명확하게 정의하고, 각 상태에 따른 동작을 체계적으로 관리할 수 있도록 도와줍니다.

가스브레이커의 상태는 다음과 같이 정의할 수 있습니다. 초기 상태는 OPENED, CLOSED, STOPPED 중 하나입니다.
- OPENED: Open 상태, PWM 신호 출력 없음
- OPENING: Open 동작 중. CW 방향으로 PWM 신호 출력
- CLOSED: Close 상태, PWM 신호 출력 없음
- CLOSING: Close 동작 중. CCW 방향으로 PWM 신호 출력
- STOPPED: Stop 상태, PWM 신호 출력 없음

현재 가스브레이커 상태는 dialGB의 value 값과, btGBOpen 및 btGBClose 버튼이 눌러졌는지 isChecked()로 확인하면 알 수 있씁니다.

```python
    @property
    def gb_fsm(self):
        val = self.dialGB.value()
...
- OPENED -> CLOSING: Close 명령 수신 시
- CLOSING -> CLOSED: 7초 경과 후
- CLOSED -> OPENING: Open 명령 수신 시
- OPENING -> OPEN: 7초 경과 후
- (OPENING, CLOSING) -> STOPPED: Stop 명령 수신 시

각 상태에 따른 동작은 다음과 같습니다. 
- OPENING: OPENED가 아닌 상태에서 btGBOpen 버튼을 클릭하면 dialGB의 value 값을 1 감소한 후 타이머 시작
- CLOSING: CLOSED가 아닌 상태에서 btGBClose 버튼을 클릭하면 dialGB의 value 값을 1 증가한 후 타이머 시작
- STOPPED: OPENED나 CLOSED가 아닌 상태에서 btGBStop 버튼을 클릭하면 타이머 중지
- OPENED, CLOSED: 타이머 슬롯에서 dialGB의 value 값이 최소값 또는 최대값이면 타이머 중지
  
가스브레이커의 최초 상태는 OPENED이며, 이 상태에서 Close 버튼을 선택(CLOSING)하면 dialGB의 value는 증가하고, Open 버튼을 선택(OPENING)하면 감소해야 합니다.   

```python
    def onTimer(self):               
        if self.gb_fsm == "OPENING":
            self.dialGB.setValue(self.dialGB.value() - 1)
        elif self.gb_fsm == "CLOSING":
            self.dialGB.setValue(self.dialGB.value() + 1)
        elif self.gb_fsm == "OPENED" or self.gb_fsm == "CLOSED":            
            self.statusbar.showMessage(self.gb_fsm)
            self.mqtt.publish("asm/iot/total/gasbreaker", {"group":None, "action":"stop"})
            self.timer.stop()
```

```python
    def onGBOpenClicked(self, checked):
        if not checked:
            self.btGBOpen.setChecked(True)
            return
        
        if self.gb_fsm == "OPENED":
...
```python
    def onGBStopClicked(self, checked):
        if not checked:
            self.btGBStop.setChecked(True)
            return
        
        if self.gb_fsm == "OPENED" or self.gb_fsm == "CLOSED":
            self.btGBStop.setChecked(False)
            return
...
```python
    def onGBCloseClicked(self, checked):
        if not checked:
            self.btGBClose.setChecked(True)
            return
        
        if self.gb_fsm == "CLOSED":
            self.btGBClose.setChecked(False)
            return
...
**6단계: 토픽 발행에 따른 결과 구현**   
토픽 메시지를 발행하기 위해 MQTT 클라이언트 객체의 publish()를 호출하면, MQTT 클라이언트는 브로커에 토픽을 전달한 후 응답을 받으면 onPublish()를 호출합니다.  
currentMessage()로 상태표시줄에 출력된 기존 문자열을 읽은 후 이전 토픽 발행 정보 부분만 새로운 정보로 대체한 다음 다시 상태표시줄에 출력합니다.     
...
UI에서는 최초 가스브레이커의 상태가 열림으로 가정하므로 실제 가스브레이커도 수동으로 열림으로 변경합니다. 
