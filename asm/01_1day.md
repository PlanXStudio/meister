...
7. 마이크로파이썬 장치 제어 툴(xnode)과 Digi XBeee 라이브러리
```sh
...
## 파워쉘7 설치
> VSCode의 터미널창과 결합되어 기존 파워쉘보다 더 강력한 기능 제공
...
2. 새로 설치한 파워쉘7(pwsh)을 실행한 후 확장 모듈 설치
...
3-1. 홈 폴더 아래 문서(Documents) 폴더로 이동한 후 PowerShell 폴더가 있는지 확인
   ```sh
   cd ~\Documents
   ls PowerShell
   ```
3-2. 만약 PowerShell 폴더가 없으면 만들기
   ```sh
   md PowerShell
   ```
3-3. ~/PowerShell 폴더에 Microsoft.PowerShell_profile.ps1 파일 생성
   ```sh
   cd PowerShell
   echo '' > Microsoft.PowerShell_profile.ps1
   ```

4. 메모장으로 ~/PowerShell/Microsoft.PowerShell_profile.ps1 파일을 열고, 앞서 설치한 모듈 로드 명령 추가
```sh
