...
- 운영체제는 저장소에서 파일을 관리할 때 폴더 단위로 묶어 트리 구조로 관리
  ```sh
  C:\VSCode
       |--- data
       |--- ext     
             |--- python
  ```  
- 트리 구조는 최상위 폴더인 루트(\)부터 시작하며, 그 아래 하위 폴더를 가짐
  ...
  - 예약 문자
    ...
    - 홈 폴더: ~
      ...
      - 윈도우 운영체제는 C:\User 아래 로그인 이름의 하위 폴더
- 경로(Path)는 트리 구조에서 루트 또는 현재 폴더에서 작업 폴더까지의 계층 구조(위치)를 나타냄
  - 현재 작업 폴더는 사용자가 작업을 수행하기 위해 이동한 폴더
     - python 또는 ./python
...
- 폴더 이동은 cd 명령 사용
  ...
  cd c:\
  ```
- 폴더에 포함된 파이 또는 하위 폴더 확인은 dir 또는 ls 명령 사용
  ```sh
  cd ~
  dir
  cd c:\
  ls
  ```
- 현재 폴더의 절대 경로 확인은 pwd 명령 사용
  ```sh
  cd ~
  pwd
  ```
- 핸재 폴더 아래 새 폴더 생성은 mkdir 또는 md 명령 사용
  ```sh
  cd c:\
  mkdir VS_Code
  ls  
  ```
- 폴더 이름 변경은 move 또는 mv 명령 사용
  ```sh
  cd c:\
  mv VS_Code VSCode
  ls
  ```
- 폴더 삭제는 rmdir 또는 rm 명령 사용
  ```sh
  cd c:\
  rm VSCode
  ls
