...
## HOME 자동화
### 케이블링
> Auto 제어기의 릴레이 2와 3의 COMMON 포트에는 기본적으로 내부 12V 출력이 연결되어 있음.
>> 12V 전원 입력을 요구하는 액추에이터를 릴레이의 NO(Normal Open) 포트에 연결해 사용

- 디지털 IO 12V 출력을 릴레이1의 COMMON에 연결
- FAN의 VCC(red)를 릴레이1의 NO에 연결
- Light1의 VCC(red)를 릴레이2 NO에 연결
- Light2의 VCC(red)를 릴레이3 NO에 연결
- FAN, Light1, Light2의 GND(block)을 각각 디지털 IO의 GND에 연결


### Auto 제어기용 마이크로파이썬 스크립트 
> 앞서 설치한 pop 및 auto 라이브러리 사용
...
**실행**
```sh
xnode -p<COM 포트> run home.py
```
- 스트립트를 실행한 후 PC에서 해당 문자열을 입력하면 Auto 제어기로 전송됨
  - 오타 수정 기능이 없으므로 오타가 발생하면 \<ENTER\> 후 다시 입력할 것
  - 사전에 정의한 문자열 외에 문자열을 전송하면 오류 메시지가 수신됨
- Auto 제어기는 PC에서 전송함 문자열을 분석해 해당 액추에이터 제어 
- 사전 정의된 제어 문자열
  - fan on
  - fan off
  - light1 on
...
### PC용 간단한 통신 파이썬 스크립트 
> xnode를 이용한 문자열 전송 대신 pyserial 라이브러리를 사용해 제어 명령을 전달하는 간단한 통신 프로그램 작성
...
PORT = "COM4" #자신의 COM 포트 번호로 변경할 것
...
def main():
    ...
    ser.write("light1 on\r".encode())
    sleep(1)
    ser.write("light1 off\r".encode())
    sleep(1)
    
    ser.write("light2 on\r".encode())
    sleep(1)
    ser.write("light2 off\r".encode())
    sleep(1)
...
**실행**
> 반드시 xnode를 종료한 상태에서 실행해야 함.

```sh
python home_ctrl.py
```

- 1초 간격으로 FAN, Light1, Light2를 차례로 켜고 끔.


### PC용 MQTT 홈 게이트웨이 
> 인터넷을 통해 MQTT 브로커로부터 구독한 메시지를 시리얼 통신으로 Auto 제어기에 전달
...
**실행**
> 반드시 xnode나 home_ctrl을 종료한 상태에서 실행해야 함.

```sh
python home_gateway.py
```

- MQTTX 클라이언트를 통해 iot/home/<자신의 고유 번호> 토픽에 Auto 제어기의 제어 문자열을 메시지로 발생
