...
## 사전 설치
1. 압축 파일 다운로드
   > [사전 설치 파일](https://1drv.ms/u/s!AtTAtBZJQ9JFoJIPhxKFaYAXTe03Kw?e=GZh2yu)
2. C:\에 압축해제
   > ```C:\VSCode```  
4. 시스템 환경 변수 PATH에 python 및 pip 실행 경로 추가
   - 관리자 권한으로 cmd 실행
   - 환경변수 PATH 수정 (주의: 반드시 1회만 실행할 것!!!)
   ```sh
   setx /m path "C:\VSCode\ext\python";"C:\VSCode\ext\python\Scripts";%PATH%
   ```

<details>
<summary>윈도우 CLI 명령</summary>
