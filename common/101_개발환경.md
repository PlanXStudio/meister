...
```
1-3. 환경변수 PATH 수정 (주의: 반드시 1회만 실행할 것!!!)
```sh
setx /m path "C:\VSCode\ext\python";"C:\VSCode\ext\python\Scripts";%PATH%
```
1-4. 만약 환경변수 PATH를 잘못 수정했다면 백업을 이용해 복원 후 다시 시도
```sh
type path.backup
setx /m path {위 출력 내용 붙여넣기}
```
 
2-1. Win+r을 누른 후 다음 명령으로 실행해 시스템 속성 실행
   ```sh
   sysdm.cpl
   ```
2-2. 고급 탭을 선택한 후 **환경 변수** 선택
2-3. \<시스템 변수\> 목록에서 **Path** 항목을 찾아 더블클릭
2-4. 목록 아래 빈 칸을 더블클릭한 후 다음 내용을 차례로 입력
   ```sh 
   C:\VSCode\ext\python
   ```
   ```sh 
   ...
   ```   
2-5. 추가한 2개 항목을 왼쪽 "위로 이동" 버튼을 이용해 목록 가장 앞으로 이동 
   ```sh
   C:\VSCode\ext\python
   C:\VSCode\ext\python\Scripts
   C:\WINDOWS\system32
   ...
   ```
* CLI 환경을 재 시작하면 임의 경로에서 python 또는 pip 명령을 사용할 수 있음
...
1. 관리자 권한으로 명령 프롬프트(cmd)를 실행
   - winget 명령 확인 
   ```sh
   winget --version
   ```

   - 만약 winget 명령이 없으면 깃 허브의 microsoft/winget-cli에서 최신 버전의 설치 파일을 다운로드한 후 설치
     - [microsoft/winget-cli](https://github.com/microsoft/winget-cli/releases)
     - Assets 목록에서 확장자가 .msixbundle인 파일을 다운로드한 후 파일을 더블클릭해 설치

3. 윈도우 패키지 관리자인 winget으로 해당 프로그램 설치
...
4. 새로 설치한 파워쉘7(pwsh)을 실행한 후 현재 설정된 모듈 경로 확인
   ```sh
   $env:PSModulePath -split ';'    
   ```
   - 2-1) OneDrive에 문서폴더를 연결하지 않은 사용자
     - 홈 폴더(C:/Users/\<login-name\>) 아래 **Documents** 폴더 아래 PowerShell 경로 사용  
    - 2-2) OneDrive에 문서폴더를 연결한 사용자
     - 홈 폴더(C:\Users/\<login-name\>) 아래 **OneDrive/문서** 폴더 아래 PowerShell 경로 사용  

5. 윈도우 시작 메뉴에서 앞서 설치한 파워쉘(powershell) 7을 실행한 후 확장 모듈 설치
...
6. 파워쉘7 시작 스크립트 작성
   - 파워셀 폴더로 이동
     - OneDrive에 문서폴더를 연결하지 않은 사용자
       ```sh
       cd ~/Documents/PowerShell
       ```
  
     - OneDrive에 문서폴더를 연결한 사용자
       ```sh
       cd ~/OneDrive/문서/PowerShell
       ```
