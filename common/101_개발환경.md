...
2. cmd을 실행한 후 winget 버전 확인(v1.7.10861 이상)
```sh
winget --version
```
...
5. 새로 설치한 파워쉘7(pwsh)을 실행한 후 파워쉘7 모듈 경로 확인
   - 모듈 경로 확인
   ```sh
   $env:PSModulePath -split ';'    
   ```
   - 첫 줄이 홈 폴더(C:\Users\\<login-name\>) 아래 **Documents\PowerShell\Modules**
     - OneDrive에 문서폴더를 연결하지 않은 사용자로 홈 폴더 아래 OneDrive 폴더가 없음
   ```sh
   cd 
   ```
   
   - 첫 줄이 홈 폴더(C:\Users\\<login-name\>) 아래 **OneDrive\문서\Modules** 
     -  OneDrive에 문서폴더를 연결 중이거나 했던 사용자로 홈 폴더 아래 OneDrive 폴더가 있음
   ```sh
   ```
   
7. 윈도우 시작 메뉴에서 앞서 설치한 파워쉘(powershell) 7을 실행한 후 확장 모듈 설치
