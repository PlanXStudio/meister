...
1. 프로그램을 시작하려면 먼저 PySide6 라이브러리에서 필요한 모듈들을 import 해야 합니다.
```python
import sys
...
3. 창으로 표시할 위젯을 생성합니다. QWidget은 사용자와 상호작용할 수 있는 시각적 구성 요소로, 상속을 통해 버튼, 텍스트 상자, 슬라이더, 체크박스, 윈도우 등 다양한 형태로 제공됩니다.
```python
win = QMainWindow()
...
## 시그널과 슬롯
시그널과 슬롯은 Qt의 이벤트 처리 메커니즘으로 이를 통해 다른 객체 사이 통신을 쉽게 구현할 수 있습니다.   
시그널은 이벤트가 발생할 때마다 시그널을 전송하고, 슬롯은 이러한 시그널에 연결되어 응답하는 함수입니다.  

다음은 1초마다 발생하는 난수 생성 이벤트를 rand 시그널로 전달하는 예입니다.
```python
...
MyRand 객체의 rand 시그널을 수신하는 슬롯 코드는 다음과 같습니다.
```python
import sys
from PySide6.QtWidgets import QApplication, QMainWindow
...
### 버튼 클릭 시그널 처리
위젯에는 키보드나 마우스 이벤트를 사용자가 처리할 수 있도록 다양한 시그널을 정의하고 있습니다.
