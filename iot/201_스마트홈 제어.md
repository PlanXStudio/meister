...
## 실습장비 기본 제어
### LED 켜고 끄기
실습장비는 프로그래밍 가능한 1개의 사용자 LED를 제공하며, 이를 이용해 내부 상태을 알림
...
### 빛(조도) 센서값 읽기
빛 센서는 스마트폰에 포함된 것과 유사한 부품으로 주변 빛의 밝기를 룩스(Lux) 단위로 제공 
...
### 배터리 잔량 읽기 
배터리로 운영되는 XNode 모트는 배터리 잔량을 확인하는 것이 중요하며, 배터리가 부족하면 충전하도록 LED 등으로 사용자에게 알릴 필요가 있음
...
```sh
xnode --sport com11 run -n serial_xnode.py
```
